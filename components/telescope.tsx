...
export default function Telescope() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      fronts.forEach((el, i) => gsap.set(el, { scale: FRONT[i] ?? 0.15, filter: 'blur(6px)' }));
      gsap.set(smalls, { transformStyle: 'preserve-3d', force3D: true });
      gsap.set(msg, { y: 24 });
      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: section, start: 'top top', end: () => '+=' + window.innerHeight * 3.4,
      ...
      tl.to(smalls, { z: '120vh', rotateZ: (i: number) => (i % 2 ? 4 : -4), duration: 1, ease: 'power1.inOut', stagger: { amount: 0.24, from: 'center' } }, 0)
        .to(fronts, { scale: 1, duration: 1, ease: 'power1.inOut' }, 0.6)
        .to(fronts, { filter: 'blur(0px)', duration: 1, ease: 'power1.inOut', stagger: { amount: 0.22, from: 'end' } }, 0.6)
        .to(msg, { opacity: 1, y: 0, duration: 0.4 }, 0.85);
    }, section);
  ...
  if (reduce) {
    return (
      <section className="relative grid place-items-center" style={{ height: '80svh', background: 'var(--ink)', overflow: 'hidden' }}>
        <img src={CENTER_IMG} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', opacity: 0.5 }} />
        <div className="center" style={{ position: 'relative', zIndex: 2, padding: '0 1.5rem' }}>
          <h2 className="display display--lg">Step Into <span className="accent">Luxury</span></h2>
